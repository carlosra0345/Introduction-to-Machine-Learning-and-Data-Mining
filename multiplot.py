...
#     plt.bar(r, np.std(X, 0), color='purple')  # Using purple for standard deviation bars
...
#         C = len(classNames)
#         for c in range(C):
#             # Ensure that y and Z have the same number of rows
#             plt.plot(Z[y == c, i], Z[y == c, j], ".", alpha=0.5)
#         plt.xlabel("PC" + str(i + 1))
#         plt.ylabel("PC" + str(j + 1))
#         plt.title(titles[k] + "\nProjection")
#         plt.legend(classNames, loc='upper right', bbox_to_anchor=(1.2, 1), fontsize='small')
#         plt.axis("equal")

#         # Plot attribute coefficients in principal component space
#         plt.subplot(nrows, ncols, 3 + k)
#         for att in range(V.shape[1]):
#             plt.arrow(0, 0, V[att, i], V[att, j], color='black')
...
#         # Add a unit circle
#         plt.plot(np.cos(np.arange(0, 2 * np.pi, 0.01)), np.sin(np.arange(0, 2 * np.pi, 0.01)), color='gray')
...
#         plt.plot(range(1, len(rho) + 1), np.cumsum(rho), "o-", color='orange')
...
def multiplot(X, attributeNames, pca, N, targets):
    ...
    for k in range(2):
        ...
        plt.subplot(3, 2, 1 + k)
        C = len(classNames)
        colors = plt.cm.plasma(np.linspace(0, 1, C))  # Use plasma color palette for classes
        for c in range(C):
